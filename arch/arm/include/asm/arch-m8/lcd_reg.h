...
#define WRITE_DPTX_REG(reg, val) *(volatile unsigned *)LCD_DPTX_ADDR(reg) = (val)
#define READ_DPTX_REG(reg) (*(volatile unsigned *)LCD_DPTX_ADDR(reg))
#define WRITE_DPTX_REG_BITS(reg, val, start, len) \
	WRITE_DPTX_REG(reg, (READ_DPTX_REG(reg) & ~(((1L<<(len))-1)<<(start))) | ((unsigned)((val)&((1L<<(len))-1)) << (start)))
