...
static struct ddr_set __ddr_setting={
	...
	.t_pub1_dtar	= ((0x0 + CONFIG_DDR1_DTAR_DTCOL)|(CONFIG_DDR1_DTAR_DTROW <<12)|(CONFIG_DDR1_DTAR_DTBANK << 28)),
	...
	.t_mmc_ddr_ctrl=	(CONFIG_DDR_CHANNEL_SWITCH<<26)| //bit 27:24.   ddr chanel selection.
											                 //bit 27: 26 :
											                     //[27:26]=2'b00 : address switch between 2 ddr channel using address bit 12.
											                            //bit12 = 0 : ddr channel 0.  bit 12 = 1 : ddr channel 1.
											                            //if 2 ddr channel size is different, only switch the lower parts.
											                     //[27:26]=2'b01 : address switch between 2 ddr channel using address bit 8.
											                            //bit8 = 0 : ddr channel 0.  bit 8 = 1 : ddr channel 1.
											                            //if 2 ddr channel size is different, only switch the lower parts.
						(CONFIG_DDR_CHANNEL_SUB_SWITCH<<24)|	//[27:26]=2'b10 : no switch between the 2 channels. use bit 24 as selection which one put lower address.
																	//bit 24 = 0:   channel 0 put to the lower address.
																	//bit 24 = 1:   channel 1 put to the lower address.
																//[27:26]=2'b11 :  all address goes to one DDR channel. use bit 24 to select which one.
																	//bit 24 = 0:   use channel 0;
																	//bit 24 = 1:   use channel 1;
						(1<<16) | //keep 1			//bit 16.       bank page policy.
						(CONFIG_DDR_BIT_MODE<<15) |	//bit 15.       ddr1 channel 1 is in 16bits mode.  0 : 32bits mode. 1: 16bits mode.
						(CONFIG_DDR_BANK_SET<<13) |//bit 14:13.    ddr1 address map bank mode
						...
						(0<<12) | //keep 0				//bit 12        ddr1 rank size.  0, 1, one rank.  2 : 2 ranks.
						(CONFIG_DDR_ROW_SIZE<<10)|	//bit 11:10      ddr1 row size.  2'b01 : A0~A12.   2'b10 : A0~A13.  2'b11 : A0~A14.  2'b00 : A0~A15.
						(CONFIG_DDR_COL_SIZE<<8) |	//bit 9:8      ddr1 col size.  2'b01 : A0~A8,    2'b10 : A0~A9.
						(CONFIG_DDR_BIT_MODE <<7) |//bit 7        ddr channel 0  is in 16bits mode.  0: 32bits mode. 1 : 16bits mode.
						(CONFIG_DDR_BANK_SET <<5) |//bit 6:5.     ddr0 address map bank mode
												 		// 00 = address switch between 2 banks  bank[0] selection bits [12].
									                    // 01 = address switch between 4 banks  bank[1:0] selection bits [13:12].
									                    // 10 = address switch between 2 banks  bank[0] selection bits [8].
									                    // 11 = address switch between 4 banks  bank[1:0] selection bits [9:8].
						(0<<4) | //keep 0			//bit 4        ddr0 rank size.  0, 1, one rank.  2 : 2 ranks.
						(CONFIG_DDR_ROW_SIZE<<2) |	//bit 3:2      ddr0 row size.  2'b01 : A0~A12.   2'b10 : A0~A13.  2'b11 : A0~A14.  2'b00 : A0~A15.
						(CONFIG_DDR_COL_SIZE<<0),	//bit 1:0      ddr0 col size.  2'b01 : A0~A8,    2'b10 : A0~A9.
...
{
    unsigned int board_ver;
    board_ver = readl(P_PREG_PAD_GPIO2_I) >> 16 & 0x1F;
    serial_puts("\n Board HW Version is ");
    //serial_put_dec(board_ver);
    switch(board_ver){
        case 2 : /*2G DDR*/
            __ddr_setting.phy_memory_size = 0x80000000;
            __ddr_setting.t_pub0_dtar = 0x000003c0;
            __ddr_setting.t_pub1_dtar = 0x00000000;
            __ddr_setting.t_mmc_ddr_ctrl = 0x00ffae2e;
            serial_puts("2");
            break;
	case 0 : /*1G DDR*/
            serial_puts("0");
