...
#define BL_PWM_MIN         	30		/** brightness diminig duty_min(unit: %, positive logic) */
...
#include <amlogic/panel/lvds/EE070NA01D.h>
...
static Lcd_Power_Config_t lcd_power_on_uboot = {.type = LCD_POWER_TYPE_PMU, .gpio = LCD_POWER_PMU_GPIO0, .value = LCD_POWER_GPIO_OUTPUT_LOW, .delay = 0};
static Lcd_Power_Config_t lcd_power_off_uboot = {.type = LCD_POWER_TYPE_PMU, .gpio = LCD_POWER_PMU_GPIO0, .value = LCD_POWER_GPIO_INPUT, .delay = 0};
