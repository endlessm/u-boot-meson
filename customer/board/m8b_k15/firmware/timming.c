...
#if (CFG_DDR_CLK >= 336 ) && (CFG_DDR_CLK <533)
	#define DDR3_7_7_7
...
#endif
#ifdef CONFIG_LPDDR2 || CONFIG_LPDDR3
#undef DDR3_7_7_7
#undef DDR3_9_9_9
#undef DDR3_11_11_11
#endif
...
#ifdef LPDDR2
//lpddr2: ELPIDA B4432BABH-1D-F
	//DTPR0
	#define CFG_DDR_RTP 4 //0 //(4)  //pctl need use 0
	#define CFG_DDR_WTR (4)
	#define CFG_DDR_RP 12 // 10   // (10)   //
	#define CFG_DDR_RPAB  12 // 10 // 12
	#define CFG_DDR_RPPB  10// 8 // 10
	#define CFG_DDR_RCD 10 //8 //(10)
	#define CFG_DDR_RAS (23)
	#define CFG_DDR_RRD (6)
	#define CFG_DDR_RC  (36)

	//DTPR1
	#define CFG_DDR_PCTL_MRD (5)  //pctl //pctl pub not same ?
	#define CFG_DDR_PUB_MRD (1)  //pctl //pctl pub not same ?
	#define CFG_DDR_MOD (0)
	#define CFG_DDR_FAW (32)
	#define CFG_DDR_RFC   112// 208 //(128) pctl limit lpddr2 cant not big
	#define CFG_DDR_WLMRD  0 //(40)
	#define CFG_DDR_WLO  0 // 7 // (6)  //jiaxing add

	//DTPR2
	#define CFG_DDR_XS   (512)
	#define CFG_DDR_XP   (5)
	#define CFG_DDR_CKE  (3)
	#define CFG_DDR_DLLK (512)
	#define CFG_DDR_RTODT (0)
	#define CFG_DDR_RTW   (4)  //pctl

	#define CFG_DDR_REFI   39//(78)
	#define CFG_DDR_REFI_MDDR3  0 //(4)
		
	#define CFG_DDR_CL    8 //(6)
	#define CFG_DDR_WR   8 // 8 //(12)
	#define CFG_DDR_CWL  4 // (8)
	#define CFG_DDR_AL    (0)
	#define CFG_DDR_EXSR  (117)
	#define CFG_DDR_DQS   4 // for g2 phy (2)
	#define CFG_DDR_CKSRE (0)
	#define CFG_DDR_CKSRX (2)
	#define CFG_DDR_ZQCS  50 // (48)
	#define CFG_DDR_ZQCL  (192)  //jiaxing add
	#define CFG_DDR_XPDLL (0)  // pctl
	#define CFG_DDR_ZQCSI (1000)
	#define CFG_DDR_TDQSCK  (1)
	#define CFG_DDR_TDQSCKMAX  (1)
	#define CFG_DDR_TCKESR   (8)
	#define CFG_DDR_TDPD      (3500)
...
static struct ddr_set __ddr_setting={
	...
	.t_ddr_apd_ctrl = ( 32 << 0 ) | //[B7..B0]: DMC active latency.  latency to enter LOW POWER state after the the DMC is not actived.
					  ...
					  ( 0x01  << 16) 	//[B17,B16]: DDR mode to genreated the refresh ack signal and prcharge signal to DMC for dmc refresh control.
					 #if defined(CONFIG_LPDDR2_RANK1)
						|( 0x1  << 19),    //use lpddr2_3 rank2 //jiaxing add
					 #endif
...
#else
				(1) |
#endif //[B0] mem-bl: 0 for 4; 1 for 8
...
#ifdef LPDDR2
				(0x2F<< 24)| (0x3 << 6)|(0x1 << 20)|(3 << 22)|     // lpddr2 -s4 burst length 4
#endif
#ifdef LPDDR3
				(0x2F<< 24)| (0x2 << 6)|(0x2 << 20)|(1 << 22)|     //
#endif
	...
	.t_pctl_mcfg1 = (1<<31)| //[B31]hw_exit_idle_en
		...
		//.t_pub_dxccr = 4|(0x91<<5)|(3<<19),
		//.t_pub_zq0pr = (0x1a)|(0x9<<8)|(0x9<<12)|(0x1<<16),
		//.t_pub_zq0pr = (0x09)|(0x9<<8)|(0x9<<12)|(0x1<<16),
		.t_pub_zq0pr = (0x19)|(0x8<<8)|(0x9<<12)|(0x1<<16),
		.t_pub_zq0dr = (0x1<<28)|(0xb<<21)|(0xb<<14)|(0x28<<7)|(0x28<<0),
#else
#ifdef LPDDR2
		.t_pub_dxccr = 4|(0xf7<<5)|(3<<19),
		.t_pub_zq0pr = 0x59,
...
#if defined (LPDDR2)||defined (LPDDR3)
		.t_pctl_tmrd   = CFG_DDR_PCTL_MRD,
		.t_pctl_trp    = ((CFG_DDR_RPAB-CFG_DDR_RPPB)<<16)|CFG_DDR_RPPB,
#else
		.t_pctl_tmrd   = CFG_DDR_PCTL_MRD,
...
#if defined (LPDDR2)||defined (LPDDR3)  //jiaxing 20140108
	.t_pctl_trtp   = 0,
#else
	.t_pctl_trtp   = CFG_DDR_RTP,
#endif
...
#if defined (LPDDR2)||defined (LPDDR3)
	.t_pctl_tckesr =CFG_DDR_TCKESR,
	.t_pctl_tdpd  =CFG_DDR_TDPD,
#endif
	...
	.t_pub_mr={
       ...
       .t_pub_mr={
		[0]=	(0 << 0 ),       						      
       	[1]=(2<< 0)|(1<< 3)|(0 << 4)|((CFG_DDR_CWL-2)<< 5),	//LPDDR2 burst length 4
           	[2]=((CFG_DDR_CL-2)<< 0),//((CFG_M8_DDR_CL-2)<< 0)//2
           	[3]=	(3 << 0 ),   
           	},
#endif
	...
	.t_pub_dtpr={
...
#ifdef LPDDR2 
            [1] =  ( ((CFG_DDR_PUB_MRD) << 0 ) |	 //tMRD      //4 TCK
		...
		[2] =  ((CFG_DDR_XS  << 0 ) |    		//tXS        //MAX(tXS,tXSDLL),tXS=170000ps,tXSDLL=512 TCK
				...
				(0 << 31)),
#if defined( LPDDR2 )||defined( LPDDR2 )
		[3] = (CFG_DDR_TDQSCK)|(CFG_DDR_TDQSCKMAX<<3),   //LPDDR2 3 DEBUG
#else
		[3] = 0,   //LPDDR2 3 DEBUG
#endif
	},
