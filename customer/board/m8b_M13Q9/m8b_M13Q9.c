...
static int _saradc_key_pressed(void)
{
        ...
        return  (0 == run_command("saradc get_in_range 0x20 0x35 1", 0));
}
...
void magic_checkstatus(int saveEnvFlag)
{
      ...
      /*  if(!update_key_num)
                for(;update_key_num < 3;)
                {
                        ...
                        if(pwrkeypressed) break;//quit as user release the powerkey
                        {
                                while(get_timer(start) < delay){
                                        if(ctrlc())return;
                                        if( 0 < run_command("getkey", 0)){
