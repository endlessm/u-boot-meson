...
void magic_checkstatus(int saveEnvFlag){
	...
	if( 0 == run_command("getkey", 0)){
		power_key_num++;
		run_command("msleep 300", 0);
	}
    else if(power_key_num)//if powerkey released but pressed ago 
    {
        power_key_num = 0;//clear it as now released
        update_key_num ++;
    }
	...
	if(saveEnvFlag){
		if(update_key_num >= 3){
			...
			run_command("run prepare; bmp display ${upgrade_logo_offset};run update", 0);
			return;
		}
