...
/*
 *config for mango led ,sys_led =0 RED LED,sys_led=1 GREEN LED
 */
#define CONFIG_MANGO_LED
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"outputmode=1080p\0" \
	"vdac_config=0x10\0" \
	"bootargs=init=/init console=ttyS0,115200n8 no_console_suspend quiet\0" \
	...
	"preboot="\
        ...
        "run prepare;"\
        "run storeargs;"\
        "get_rebootmode; if test ${reboot_mode} != power_off;then clear_rebootmode; echo reboot_mode=${reboot_mode}; fi; " \
        "run update_key; " \
	...
	"switch_bootmode="\
        ...
        "else if test ${reboot_mode} = usb_burning; then "\
        	"run usb_burning;"\
        "else if test ${reboot_mode} = power_off; then "\
        	"echo enter_suspend; suspend;"\
        "else " \
        	"  "\
        "fi;fi;fi;fi\0" \
    \
