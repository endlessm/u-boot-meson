...
#undef CONFIG_CMD_SARADC
#define CONFIG_EFUSE 1
//#define CONFIG_MACHID_CHECK 1
#define CONFIG_CMD_SUSPEND 1
#define CONFIG_IR_REMOTE_RECOVERY
...
#define CONFIG_EXTRA_ENV_SETTINGS \
    ...
    "update_key="\
        "echo update by key...; " \
        "if irdetect; then run recovery; fi\0" \
   	"update="\
    ...
    \
   	"storeargs="\
        "setenv bootargs init=/init console=${console} no_console_suspend vdaccfg=${vdac_config} storage=${store} logo=osd1,loaded,${fb_addr},${outputmode},full hdmimode=${hdmimode} cvbsmode=${cvbsmode} androidboot.firstboot=${firstboot} hdmitx=${cecconfig} val1=${sdioclk} sdhc_debug=0x00000200 rx_clk_phase=${rx_clk_phase} sd_clk_phase=${sd_clk_phase}\0"\
    \
	"switch_bootmode="\
...
//#define CONFIG_DDR_512MB
//#define CONFIG_DDR_1GB
#define CONFIG_DDR_2GB

/*DON'T MODIFY FOLLOWING ROW/COL SETTINGS*/
/*ROW/COL SETTINGS END*/
//row size.  2'b01 : A0~A12.   2'b10 : A0~A13.  2'b11 : A0~A14.  2'b00 : A0~A15.
//col size.   2'b01 : A0~A8,      2'b10 : A0~A9
#define GET_DDR_ROW_BITS(mem_size, channel_set, bus_width) (13+(mem_size>>30)+ \
	((channel_set>=CFG_DDR_ONE_CHANNEL_DDR0_ONLY)?(1):(0))+((bus_width>=CFG_DDR_BUS_WIDTH_16BIT_LANE01)?(1):(0)))
...
#elif !defined(CONFIG_DDR_1GB) && !defined(CONFIG_DDR_2GB) && !defined(CONFIG_DDR_512MB)
	#error "Please set DDR capacity first in board config file!\n"
#endif
#define CONFIG_DDR_COL_BITS  (10)
#define CONFIG_DDR_COL_SIZE  (2)
#define CONFIG_DDR_ROW_BITS GET_DDR_ROW_BITS(PHYS_MEMORY_SIZE, CONFIG_DDR_CHANNEL, CONFIG_DDR_MODE)
#define CONFIG_DDR_ROW_SIZE GET_DDR_ROW_SIZE(CONFIG_DDR_ROW_BITS)
/*ROW/COL SETTINGS END*/
