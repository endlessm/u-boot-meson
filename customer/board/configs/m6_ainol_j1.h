...
#define BATTERYCAP				3800							//battery capability
#endif
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"upgrade_check=if itest ${upgrade_step} == 0; then defenv; save; run update; else if itest ${upgrade_step} == 1; then defenv; setenv upgrade_step 2; save; poweroff; fi; fi\0" \
	"switch_bootmode=get_rebootmode; clear_rebootmode; echo reboot_mode=${reboot_mode}; if test ${reboot_mode} = normal; then run prepare; mw.b ${fb_addr} 0xff 200000; bmp display ${poweron_offset}; else if test ${reboot_mode} = factory_reset; then run recovery; else if test ${reboot_mode} = update; then run update; else run charging_or_not; fi; fi; fi\0" \
	"prepare=nand read logo ${loadaddr_misc} 0 60000; unpackimg ${loadaddr_misc}; video open; video clear; video dev bl_on\0" \
	"update=run prepare; mw.b ${fb_addr} 0xff 200000; bmp display ${bootup_offset}; if mmcinfo; then if fatload mmc 0 ${loadaddr} aml_autoscript; then autoscr ${loadaddr}; fi; if fatload mmc 0 ${loadaddr} uImage_recovery; then setenv bootargs ${bootargs} a9_clk_max=800000000; bootm; fi; fi; nand read recovery ${loadaddr} 0 400000; setenv bootargs ${bootargs} a9_clk_max=800000000; bootm\0" \
	"recovery=run prepare; mw.b ${fb_addr} 0xff 200000; bmp display ${bootup_offset}; if nand read recovery ${loadaddr} 0 400000; then setenv bootargs ${bootargs} a9_clk_max=800000000; bootm; else echo no uImage_recovery in NAND; fi\0" \
	"charging_or_not=if ac_online; then run prepare; run charging; else if getkey; then run prepare; mw.b ${fb_addr} 0xff 200000; bmp display ${poweron_offset}; run bootcmd; else poweroff; fi; fi\0" \
...
#define CONFIG_BOOTCOMMAND  "mw.b ${fb_addr} 0xff 200000; bmp display ${bootup_offset}; nand read boot ${loadaddr} 0 400000; setenv bootargs ${bootargs} a9_clk_max=800000000; bootm"
...
//note: please DO NOT remove following check code
	...
	#error "Please set DDR3 property first in file m6_ainol_j1.h\n"
#endif
...
//note: please DO NOT remove following check code
	...
	#error "Please set DDR3 capacity first in file m6_ainol_j1.h\n"
#endif
...
#else
	#error "Please define DDR3 memory capacity in file m6_ainol_j1.h\n"
#endif
